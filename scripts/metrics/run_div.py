...
for folder in binnings:
    ...
    ##########################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    alleles_lists = []
    for i in range(len(seq_list_base_complete)):
        alleles = []
        for j in range(len(seq_list_base_complete[i])):
            mutant = seq_list_base_complete[i][j][1]
            alleles.append(mutant)
        alleles_lists.append(alleles)
    
    LTraj = len(alleles_lists)
    alleles_traj = []
    alleles_names = []
    for i in range(len(alleles_lists)):
        for j in range(len(alleles_lists[i])):
            if (alleles_lists[i][j] not in alleles_names):
                #print(alleles_lists[i][j])
                alleles_names.append(alleles_lists[i][j])
                zeros = np.zeros(LTraj)
                zeros[i] += 1
                alleles_traj.append(zeros)
            else:
                index = np.where(np.array(alleles_names) == alleles_lists[i][j])[0][0]
                zeros = alleles_traj[index]
                zeros[i] += 1
                alleles_traj[index] = zeros
    
    xax = np.arange(0,LTraj)
    fig, ax = plt.subplots()
    for column,label in zip(alleles_traj,alleles_names):
        ax.plot(xax,column, label=label)
    ax.set_xlabel('bin')
    ax.set_ylabel('Frequency')
    #ax.legend()
    fig.tight_layout()
    name = str(out_dir)+"/plot_allele_freq_"+folder+"_bin_sizes.png"
    ...
    analyze = analyzeTrajectory(filtered_seqset, mut_proportion, reference)
    #Theta from mutation frequencies - LS
    thetas2, mus, num_seqs, variances = analyze.analyzeBinsF()
    # Tajima's D
    thetas_pi, thetas_wat, tajimas_d, variance_wat, variance_pi = analyze.analyzeBinsSegrS()
    # Tajima's D global
    # Merge the bins
    filtered_seqset_merged = [item for subset in filtered_seqset for item in subset]
    analyze = analyzeTrajectory([filtered_seqset_merged], mut_proportion, reference)
    thetas_pim, thetas_watm, tajimas_dm, variance_watm, variance_pim = analyze.analyzeBinsSegrS()
    ...
    # Linear model - with delta t as number of days between bin means
    # Number of days between two bins
    days_between_bins = []
    for i in range(len(mean_header_bin)-1):
        dates_dt = np.array([mean_header_bin[i],mean_header_bin[i+1]], dtype='datetime64[s]')
        delta_days = (dates_dt[1]-dates_dt[0])/np.timedelta64(1,'D')
        days_between_bins.append(delta_days)
    
    # Call the regression
    """
    Call the LS model and bootstrap
    """
    thetas = analyze.calculateParamsFromLS(thetas2,mus,days_between_bins)
    print(thetas)
    # Call bootstrap
    # Bootstrap alleles over all bins or only inside each 
    #print("Starting bootstrap...")
    
    #bootstrap = BootstrapFP(seq_list_base_complete,100,days_between_bins,reference)
    #thetasB = bootstrap.run()
    #print("Done.")
    
#    ordered_bin_bootstraps = [[] for k in range(len(thetasB[0]))]
#    for i in range(len(thetasB[0])):
#        for j in range(len(thetasB)):
#            ordered_bin_bootstraps[i].append(thetasB[j][i])
#    print(len(ordered_bin_bootstraps))
    
    
    # Variance by bin size
#    variance = []
#    
#    for i in range(len(thetas)):
#        variance.append(1/num_seqs[i+1])
#        
    weeks = np.arange(0,len(thetas))
    variance_size = variance
...
#    fig, ax1 = plt.subplots()
#    ax1.set_title("%s" % plot_title)
#    ax1.plot(mean_header_bin[1:], thetas, '-', color='royalblue')
#    ax1.set_xlabel('Mean bin date')
#    ax1.set_ylabel(r'kN')
#    ax1.set_xticks(mean_header_bin[1:][::10])
#    ax1.set_xticklabels(mean_header_bin[1:][::10])
#    name = str(out_dir)+"/plot_"+folder+"_thetas.png"
#    fig.savefig(str(name),dpi=300)
#    fig.clf()
    
    #thetas = np.array(thetas2[1:])/np.array(mus)
     # Write general stats table
    name_table = "table_"+folder+"_nucdiv_var_from_size.tsv"
    table_path = str(out_dir) + '/' + name_table
    with open(table_path, 'w+', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(["date","thetaWatterson","thetaPi","tajimaD", "varianceWat", "variancePi"])
        for i in range(len(thetas_pi)):
            # thetas_pi, thetas_wat, tajimas_d
            writer.writerow([mean_header_bin[i], thetas_pi[i], thetas_wat[i], tajimas_d[i],variance_wat[i], variance_pi[i]])
        writer.writerow(['total', thetas_pim[0], thetas_watm[0], tajimas_dm[0],variance_watm[0], variance_pim[0]])
    ...
    with open(table_path, 'w+', newline='') as csvfile:
        ...
        for i in range(len(weeks)):
            writer.writerow([mean_header_bin[1:][i], thetas[i], variance_size[i], variance[i], num_seqs[i], times[i], cases_on_mean_date[i], cases_on_all_dates_in_seqbin[i], num_days_per_bin[i]])
    print("      Done.\n")
